...
class Patient:
    ...
    def horizontal_plot(self, z: int):
        ...
        z, y, x = self.spacing
        aspect = y/x
        self._single_plot(img, aspect=aspect)
    
    
    def frontal_plot(self, y: int):
        img = self._image[:, y, :].T
        z, y, x = self.spacing
        aspect = x/z
        self._single_plot(img, aspect=aspect, figsize=(20, 5))
    

    def longitudinal_plot(self, x: int):
        img = self._image[:, :, x].T
        z, y, x = self.spacing
        aspect = y/z
        self._single_plot(img, aspect=aspect, figsize=(20, 5))        
    ...
    def _single_plot(self, img, aspect=1.0, figsize=(10, 10)):
